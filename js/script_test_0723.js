...
window.onload=function () {
            ...
            $.getJSON("test.json", function (data) {
                ...
                $.each(data[0].rows, function (i, item) {
                    if (item.orderCode === add) {
                        ...
                        total++;
                        //遍历del查询本页是否有被删除的数据
                        let dl=erg(del,counter);
                        //遍历inter查询本页是否有增加的数据
                        let inc=erg(inter,counter);
                        if(inc>=dl){
                            last();
                        }
    ...
    //遍历数组,返回数组中counter相等的元素数量i
    function erg(item,counter){
        let i=0;
    ...
    function Rendering() {
        ...
        //判断表格是否为空
        //alert(erg(del,counter));
        //alert(erg(del,counter)===nor);
        //alert(Math.ceil(counter / nor + 1)===Math.ceil(total / nor));
        //alert(erg(del,counter)===73%nor);
        /*
        if(erg(del,counter)===73%nor){
            counter-=nor;
            alert("我执行了");
        }
        */
        if(erg(del,counter)===nor){
            counter+=nor;
            alert("我执行了");
        }
        $.getJSON("test.json", function (data) {
